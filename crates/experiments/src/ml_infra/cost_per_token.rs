...
use std::path::Path;
use physics::{cost_per_million_tokens, rack_cost_per_sec, BYTES_PER_TOKEN, CONTEXT_LENGTH, FLOPS, GPU_COST_PER_HOUR, GPUS_IN_RACK, MEM_BW, N_ACTIVE, N_TOTAL};
use ndarray::Array1;
...
#[cfg(test)] mod tests { use super::*; use std::path::PathBuf; use std::fs;
    #[test] fn t() { let p = PathBuf::from("/tmp/test_cpt.png"); run(&p).unwrap(); assert!(p.exists()); let _ = fs::remove_file(&p); }
}
