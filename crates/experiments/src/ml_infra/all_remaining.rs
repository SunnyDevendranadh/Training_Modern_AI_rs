...
pub mod moe_routing {
    ...
    pub fn run(path: &Path) -> Result<(), Box<dyn std::error::Error>> {
        ...
        println!(
            "  Avg/expert={:.1}, CV={:.3}",
            stats.avg_tokens_per_expert, stats.coefficient_of_variation
        );
    ...
    mod tests {
        ...
        #[test]
        fn t() {
            let p = PathBuf::from("/tmp/test_moe.png");
            run(&p).unwrap();
            assert!(p.exists());
...
pub mod pipeline {
    use super::*;
    use physics::pipeline::bubble_ratio;
    pub fn run(path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    mod tests {
        ...
        #[test]
        fn t() {
            let p = PathBuf::from("/tmp/test_pipe.png");
            run(&p).unwrap();
            assert!(p.exists());
            let _ = fs::remove_file(&p);
        }
...
pub mod memory_tiers {
    ...
    mod tests {
        ...
        #[test]
        fn t() {
            let p = PathBuf::from("/tmp/test_mem.png");
            run(&p).unwrap();
            assert!(p.exists());
            let _ = fs::remove_file(&p);
        }
...
pub mod scaling_laws {
    ...
    pub fn run(path: &Path) -> Result<(), Box<dyn std::error::Error>> {
        ...
        println!(
            "  Chinchilla: {:.2e}, Over-training: {:.0}x",
            20.0 * 100e9,
            ot
        );
    ...
    mod tests {
        ...
        #[test]
        fn t() {
            let p = PathBuf::from("/tmp/test_scl.png");
            run(&p).unwrap();
            assert!(p.exists());
            let _ = fs::remove_file(&p);
        }
