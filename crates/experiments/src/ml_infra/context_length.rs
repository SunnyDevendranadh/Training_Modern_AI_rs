...
use std::path::Path;
use physics::{t_compute, t_mem_kv, t_mem_weights, BYTES_PER_TOKEN, FLOPS, MEM_BW, N_ACTIVE, N_TOTAL};
use ndarray::Array1;
...
pub fn run(path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let tm = &tw + &tk;
    let cross = (0..500).min_by(|&i, &j| (tc[i] - tm[i]).abs().partial_cmp(&(tc[j] - tm[j]).abs()).unwrap()).unwrap();
...
#[cfg(test)] mod tests { use super::*; use std::path::PathBuf; use std::fs;
    #[test] fn t() { let p = PathBuf::from("/tmp/test_cl.png"); run(&p).unwrap(); assert!(p.exists()); let _ = fs::remove_file(&p); }
}
