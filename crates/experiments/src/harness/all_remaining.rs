...
pub mod knowledge_decay {
    use super::*;
    use physics::knowledge::simulate_entropy;
    ...
    mod tests {
        ...
        #[test]
        fn t() {
            let p = PathBuf::from("/tmp/test_kd.png");
            run(&p).unwrap();
            assert!(p.exists());
...
pub mod agent_review_pipeline {
    use super::*;
    use physics::reviews::simulate_review_pipeline;
    pub fn run(path: &Path) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let full = result
            ...
            .unwrap();
        println!(
            "=== Review Pipeline: {:.1}% detection ===",
            result.errors_caught[full] as f64 / 10.0
        );
    ...
    mod tests {
        ...
        #[test]
        fn t() {
            let p = PathBuf::from("/tmp/test_arp.png");
            run(&p).unwrap();
            assert!(p.exists());
            let _ = fs::remove_file(&p);
        }
...
pub mod harness_pricing {
    ...
    mod tests {
        ...
        #[test]
        fn t() {
            let p = PathBuf::from("/tmp/test_hp.png");
            run(&p).unwrap();
            assert!(p.exists());
            let _ = fs::remove_file(&p);
        }
...
pub mod context_window_economics {
    use super::*;
    use physics::knowledge::agents_md_passive_effectiveness;
    pub fn run(path: &Path) -> Result<(), Box<dyn std::error::Error>> {
        for (ctx, name) in [
            (128000.0, "GPT-4o"),
            (200000.0, "Claude 4"),
            (1_000_000.0, "Gemini 2.0"),
        ] {
    ...
    mod tests {
        ...
        #[test]
        fn t() {
            let p = PathBuf::from("/tmp/test_cwe.png");
            run(&p).unwrap();
            assert!(p.exists());
            let _ = fs::remove_file(&p);
        }
